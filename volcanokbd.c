/* 
 * Modecom Volcano Lanparty Linux keyboard driver
 * https://bitbucket.org/dmn/volcanokbd
 */
/*
 * This driver is based on: https://bitbucket.org/Swoogan/aziokbd
 *
...
/* 120-127 */   KEY_KP8, KEY_KP9, KEY_KP0, KEY_KPDOT, KEY_102ND, KEY_COMPOSE, KEY_RESERVED, KEY_RESERVED,
        /* END 07 */
...
*/
    ...
    if (kbd->new[1] > 0 || kbd->new[2] > 0 || kbd->new[3] > 0 || kbd->new[4] > 0 || kbd->new[5] > 0 || kbd->new[6] > 0 || kbd->new[7] > 0) {
    ...
    if (kbd->new[0] == 6) {
        for (j = 1; j < 8; j++) {
            ...
            for (i = 0; i < 8; i++) {
                /* if (1 == ((kbd->new[j] >> i) & 1)) {
                    ...
                    printk(", ");
                    printk("value: ");
                    printk("%d ", az_kbd_keycode[offset + i]);
                    printk(", ");
                    printk("state: ");
                    printk("%d ", (kbd->new[j] >> i) & 1);
                    printk("\n");
                } */
        ...
        /* Computer - F11 */
        if (kbd->new[1] == 148 && kbd->old[1] != 148) {
            input_report_key(kbd->dev, KEY_FILE, 1); // KEY_COMPUTER
        }
        if (kbd->old[1] == 148 && kbd->new[1] != 148)
            input_report_key(kbd->dev, KEY_FILE, 0);
